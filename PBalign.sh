...
#   SKIPALN=1
#fi
#if [[ $SPECIAL =~ "only_map" ]]; then #skip markdup
#   SKIPMD=1
#fi
#For now we exclude marking duplicates, and only implement mapping:
SKIPMD=1
...
#   $BWA mem ${BWAOPTIONS} -t ${NUMPROCS} -R "@RG\\tID:${PREFIX}\\tSM:${PREFIX}\\tPL:ILLUMINA\\tLB:${PREFIX}" ${REF} ${READS} 2> ${OUTPUTDIR}logs/bwaMem${PREFIX}.stderr | $SAMTOOLS view -u -@ ${NUMPROCS} - 2> ${OUTPUTDIR}logs/samtoolsView${PREFIX}.stderr | $SAMTOOLS sort -@ ${NUMPROCS} -m 3G -T ${OUTPUTDIR}${PREFIX}_sorttemp -o ${OUTPUTDIR}${PREFIX}_sorted.bam 2>&1 > ${OUTPUTDIR}logs/samtoolsSort${PREFIX}.log
   ${NGMLR} -t ${NUMPROCS} -r ${REF} --skip-write -q ${READS} --rg-id "${PREFIX}" --rg-sm "${PREFIX}" --rg-lb "${PREFIX}" --rg-pl "PACBIO" 2> ${OUTPUTDIR}logs/ngmlr_${PREFIX}.stderr | ${SAMTOOLS} view -u -@ ${NUMPROCS} - 2> ${OUTPUTDIR}logs/samtoolsView_ngmlr_${prefix}.stderr | ${SAMTOOLS} sort -@ ${NUMPROCS} -m 3G -T ${OUTPUTDIR}${PREFIX}_sorttemp -o ${OUTPUTDIR}${PREFIX}_sorted.bam 2>&1 > ${OUTPUTDIR}logs/samtoolsSort_ngmlr_${PREFIX}.log
   NGMLRCODE=$?
   if [[ $NGMLRCODE -ne 0 ]]; then
      echo "ngmlr of ${READS} against ${REF} failed with exit code ${NGMLRCODE}!"
      exit 5
   fi
else
...
#Index the BAM produced by ngmlr+SAMtools:
echo "Indexing the sorted BAM for sample ${PREFIX}"
$SAMTOOLS index ${OUTPUTDIR}${PREFIX}_sorted.bam 2>&1 > ${OUTPUTDIR}logs/samtoolsIndexSorted_ngmlr_${PREFIX}.log
INDEXCODE=$?
...
echo "PBMAP complete for sample ${PREFIX}!"
